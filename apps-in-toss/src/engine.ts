// @ts-nocheck
/* ===== 사장님 몰래, 월급루팡 — 게임 엔진 (앱인토스 포팅) ===== */
export function createGame(sdk = {}) {
  // (ES 모듈은 자동 strict)
  ...
  let holding = false;
  let score = 0; // 초
  let best = Number(sdk.initialBest) || 0;
  ...
  function track(name) {
    try { sdk.track && sdk.track(name); } catch (e) { /* 무시 */ }
  }
  ...
  function gameOver() {
    ...
    if (isRecord) { best = score; try { sdk.saveBest && sdk.saveBest(best); } catch (e) {} }

    const grade = gradeFor(score);
    track("game-over/" + grade.name); // 등급별 분포 집계
    try { sdk.submitScore && sdk.submitScore(score); } catch (e) {}
    setTimeout(() => {
  ...
  const SHARE_LINK = sdk.shareLink || (IS_PUBLIC ? PLAY_URL : "");
  const SHARE_URL = sdk.shareUrl || (IS_PUBLIC ? PLAY_URL.replace(/^https?:\/\//i, "").replace(/\/$/, "") : "월급루팡 검색");
  ...
  async function shareAction() {
    ...
    if (sdk.share) {
      try { await sdk.share({ text, imageBlob: lastCardBlob }); track("share-done/toss"); return; } catch (e) { /* 폴백 진행 */ }
    }
  ...
  return { destroy, start: startGame, openLeaderboard: () => sdk.openLeaderboard && sdk.openLeaderboard() };
}
